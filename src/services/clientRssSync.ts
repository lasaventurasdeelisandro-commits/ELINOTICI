...
export async function executeClientRssSync(
  ...
  // Process feeds in parallel (up to 4 at a time)
  const feedPromises = enabledFeeds.slice(0, 6).map(async (feed) => {
    try {
  ...
  const combined = [...newlyFetchedArticles, ...existingArticles].slice(0, 50);
  ...
  if (typeof window !== 'undefined') {
    try {
      ...
      localStorage.setItem('elinoticia_last_sync_time', new Date().toISOString());
    } catch (e) {
